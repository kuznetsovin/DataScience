...
target = data[7129]
train = data.drop(7129, axis=1)

#кодируем значение текстовых параметров
LabelCode = LabelEncoder()
LabelCode.fit(target.drop_duplicates().tolist())
target = LabelCode.transform(target)
...
scores = cross_validation.cross_val_score(model, train, target, cv = kfold)
itog.insert(len(itog.columns),'Neighbors 2',scores)
...
SumInfo[SumInfo.index == 'mean'].plot(kind='bar')
